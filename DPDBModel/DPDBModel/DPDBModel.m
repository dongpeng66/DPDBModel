...
@implementation DPDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [DPDBModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    DPDBHelper *dpDB = [DPDBHelper shareInstance];
    [dpDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    DPDBHelper *dpDB = [DPDBHelper shareInstance];
    [dpDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[DPDBHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
{
    //判断是否是DPBaseModel的子类
    for (DPDBModel *model in array) {
        if (![model isKindOfClass:[DPDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [dpDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            DPDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
